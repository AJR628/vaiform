...
| GET | `/credits` | direct app route | `reqId -> cors -> getCreditsHandler` | credits handler | required (manual token check) | json | Default-Reachable | yes | no (root path is conditional fallback only for GET on `/api/*` 404) | no | Drift (`code/message`) | `src/app.js:215`, `src/handlers/credits.get.js:11-27`, `web/dist/api.mjs:156-163` |
| GET | `/api/credits` | direct app route | `reqId -> cors -> getCreditsHandler` | credits handler | required (manual token check) | json | Default-Reachable | yes | yes (`apiFetch(\"/credits\")` from dist clients maps to `/api/credits`) | yes | Drift (`code/message`) | `src/app.js:222`, `src/handlers/credits.get.js:11-27`, `web/dist/frontend.js:140`, `web/dist/creative.html:1148`, `web/dist/js/my-images.js:132`, `web/dist/api.mjs:152` |
...
| GET | `/generate` | direct app guard | `reqId -> cors -> inline` | 405 guard | public | json | Default-Reachable | yes | no | no | Drift (`code/message`) | `src/app.js:229-232` |
...
| POST | `/api/assets/ai-images` | `/api/assets` | `reqId -> cors -> requireAuth -> planGuard -> validate` | inline 410 | required | json | Default-Reachable | yes | yes (`web/dist/creative.html`) | yes | Drift (missing requestId, legacy chain still present) | `src/routes/assets.routes.js:12-17`, `web/dist/creative.html:2229` |
...
| POST | `/api/caption/preview` | `/api` | `reqId -> cors -> requireAuth -> previewRateLimit -> json(200kb)` | caption preview | required | json | Default-Reachable | yes | yes (`web/dist/creative.html` -> dynamic import `web/dist/js/caption-preview.js`) | yes | Canonical (`ok/fail`) | `src/app.js:308-310`, `src/routes/caption.preview.routes.js:101`, `web/dist/creative.html:1078`, `web/dist/js/caption-preview.js:82` |
