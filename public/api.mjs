...
try {
  if (
    ...
    (window.__VAIFORM_DEBUG__ === '1' || window.VAIFORM_DEBUG === '1')
  ) {
...
export async function apiFetch(path, opts = {}) {
  ...
  const isGeneratePost =
    path.startsWith('/generate') && (!opts.method || opts.method.toUpperCase() === 'POST');
...
try {
  if (typeof window !== 'undefined') {
    window.vaiform_diag = {
      ...
      async tokenWait(ms = 4000) {
        return waitForToken(ms);
      },
      async whoami() {
