import { BACKEND as BACKEND_FROM_CONFIG } from '/config.js';
        ...
        // âœ… Clear legacy V2 cache to prevent schema mixing
        try {
        ...
        function overlayMetaToSSOT(meta) {
            // âœ… Extract pixels computed by emitCaptionState
            return {
                ssotVersion: 3,
                mode: 'raster',
                ...
                // âœ… CANONICAL PIXELS (no percentages!)
                frameW: 1080,
                frameH: 1920,
                fontPx: meta.fontPx,
                ...
                rasterH: meta.rasterH,  // âœ… Client canonical value
                yPx_png: meta.yPx_png,
                rasterPadding: meta.rasterPadding || 24,  // âœ… Use consistent naming
        ...
        async function generateQuote() {
            ...
            if (!ENABLE_LEGACY_STUDIO && !isUrl(text)) {
                showError('quote-error', 'Legacy quote studio is disabled. Use Article Explainer for URL-based content.');
                return;
            }
        ...
        async function displayQuote(quote, options = {}) {
            ...
            document.getElementById('quote-author').textContent = quote.author ? `â€” ${quote.author}` : '';
        ...
        function updateRenderPreview() {
            ...
            if (currentQuote || selectedAsset || currentVoiceId) {
                ...
                requestAnimationFrame(() => {
                    // Ensure proper sequence: Container â†’ Canvas â†’ Preview â†’ Caption
        ...
        // Article-only: update caption size label; no overlay/render preview
        function updateCaptionStyleValues() {
            const label = document.getElementById('size-value');
            if (label && typeof getCaptionPx === 'function') {
                label.textContent = `${getCaptionPx()}px`;
            }
        }
        ...
        function buildCaptionPayload(text, captionStyle) {
            ...
            if (typeof yPct !== 'number') {
              if (window.CaptionGeom?.yPctFromPlacement) {
                ...
                yPct = 0.80; // Default to bottom if scripts fail
              }
            }
        ...
        // No-op for Article-only (quote Create button not present)
        function updateRenderButtonState() {}

        // No-op for Article-only (no live preview container)
        ...
        function computeFittedTextForPreview(text, { font, weight, sizePx, previewWidthPx }) {
            try {
                ...
                // Match overlay: max content width â‰ˆ 92% of preview width
        ...
        function normalizeRasterPayload(p) {
            ...
            return {
                ...
                // âœ… Force-fill text fields (server schema-friendly)
                ...
                // strings â€“ ensure they're present
        ...
        function readComputed(el) {
            ...
            const fontPx = Math.round(px(cs.fontSize));  // âœ… Round immediately
            const lineHeightPx = cs.lineHeight === 'normal'
                ? Math.ceil(fontPx * 1.2)             // browser default fallback
                : Math.round(px(cs.lineHeight));  // âœ… Round
            const lineSpacingPx = Math.max(0, Math.round(lineHeightPx - fontPx));  // âœ… Round
            const letterSpacingPx = Math.round(px(cs.letterSpacing));  // âœ… Round
        ...
        async function savePreview(buttonElement = null) {
            ...
            try {
                ...
                // âœ… Normalize payload for server strict validation
                ...
                // âœ… Comprehensive validation of normalized payload
                ...
                console.log('[savePreview-v3] âœ… All raster fields present (server-authoritative):', {
                ...
                // âœ… NEW: Show server PNG for WYSIWYG guarantee
        ...
        async function renderShort() {
            ...
            try {
                ...
                // 4) Voice handling (don't change existing selection logicâ€”just make sure we send strings)
                ...
                if (payload.overlayCaption) {
                  ...
                  if (Number.isFinite(fontPx) && Number.isFinite(lineSpacingPx) && 
                    ...
                    console.log('[render-INVARIANT] âœ… Client totalTextH validation passed:', { totalTextH, expectedTotalTextH });
                  }
                }
        ...
        async function oneClickShortFromLink(url, oneClickBtn, mobileOneClickBtn) {
            ...
            if (currentStorySessionId && currentStoryUrl && currentStoryUrl === url) {
                ...
                setOneClickStatus('Using existing storyâ€¦');
                if (oneClickBtn) {
                ...
                setOneClickStatus('Extracting article contentâ€¦');
                if (oneClickBtn) {
                    oneClickBtn.disabled = true;
            ...
            // Step 2: Finalize story (runs full pipeline: generate â†’ plan â†’ search â†’ render)
            setOneClickStatus('Generating storyâ€¦');
        ...
        async function oneClickShortFromQuote(oneClickBtn, mobileOneClickBtn) {
            setOneClickStatus('Generating quoteâ€¦');
            ...
            setOneClickStatus('Finding backgroundâ€¦');
            ...
            setOneClickStatus('Saving previewâ€¦');
