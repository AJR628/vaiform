...
| `POST /stripe/webhook`      | External Stripe delivery to `src/routes/stripe.webhook.js`; mounted in `src/app.js:118-118` | External billing callback, not a mobile API route.                                  | Billing correctness or entitlement updates fail for mobile users.     |
