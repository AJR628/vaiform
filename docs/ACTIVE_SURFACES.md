...
- The mobile large storyboard preview now uses backend-owned `draftPreviewV1`: backend generates one captioned preview MP4 with burned karaoke captions, and mobile plays `draftPreviewV1.artifact.url` as the ready-preview visual source. `captionOverlayV1` and `playbackTimelineV1` are retained only as compatibility/timing/style metadata, not as the ready-preview visual renderer or active mobile playback engine.
