...
| `/creative.html` (static) | `express.static("public")` | same HTML shell | `src/app.js:366`, `public/creative.html` |
...
- `creative.article.mjs` calls story endpoints and assets/options:
  ...
  - `/assets/options`
  - evidence: `public/js/pages/creative/creative.article.mjs:1047-3792` (multiple callsites)
...
- `/credits`, `/checkout/*`, `/generate`, `/job/:jobId`, `/api/shorts/:jobId`
- evidence examples:
  - `public/js/my-images.js:136`, `public/js/my-images.js:223`, `public/js/my-images.js:388`
  - `public/js/buy-credits.js:40`, `public/js/buy-credits.js:52`, `public/js/buy-credits.js:123`
...
- `/api/limits/usage` and `/limits/usage` (present callers mainly in non-default or source-only contexts): `src/app.js:279-280`, `src/routes/limits.routes.js:7`
...
- evidence: `public/js/pages/creative/creative.legacy-quotes.mjs:1308`, `:3088`, `:2995`, `:8960`
...
Yet source callers exist (not served from runtime by those mounts):
...
- Therefore, `web/src/**` is used for “present/not served” notes, not Active classification by itself.
...
- static asset/file routes via `express.static`: `src/app.js:192`, `src/app.js:343`, `src/app.js:366`

## 9) CI Truth (Surface Governance)
