...
- Backend `POST /api/story/sync`, `POST /api/story/preview`, and `POST /api/story/finalize` require `X-Idempotency-Key`, and the current mobile callers send it (`src/routes/story.routes.js`, `client/api/client.ts`).
...
- `POST /api/story/preview`
  - Mobile caller(s): `client/screens/StoryEditorScreen.tsx`, `client/screens/story-editor/useStep3SessionModel.ts`, `client/screens/story-editor/useStep3PreviewArtifact.ts`, `client/api/client.ts`
  - Backend handler(s): `src/routes/story.routes.js`, `src/services/story-preview.attempts.js`, `src/services/story-preview.runner.js`, `src/workers/story-preview.worker.js`, `src/services/story.service.js`
  - Mobile sends: `{ sessionId }` plus `X-Idempotency-Key`.
  - Backend returns: full session in `data` and may return top-level `preview: { state, attemptId }` on accepted/pending work.
  - Mobile polling rule: mobile polls only `GET /api/story/:sessionId`; there is no separate preview status route.
  - Contract rules:
