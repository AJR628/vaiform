...
| Idempotency                                         | `src/middleware/idempotency.firestore.js` plus backend-owned finalize attempt helpers in `src/services/story-finalize.attempts.js`                                     | _Removed in C12_ in-memory duplicate                              | `src/routes/story.routes.js:5`, `src/routes/story.routes.js:971`, `src/services/story-finalize.runner.js:1-243`                                                                                                                                                                  | Default-Reachable | Firestore remains the finalize-attempt SSOT: middleware owns request-time replay/reserve/enqueue semantics, while `story-finalize.attempts.js` owns shared async-safe settlement, release, lease, and stale-attempt reaping helpers. Duplicate `src/middleware/idempotency.js` was removed in C12 after import-proof confirmed no runtime route imports.                                    |
...
1. Use tracked caller evidence from `web/public` for repo-backed runtime caller proof (for example, `web/public/api.mjs`).
2. Apply API path mapping truth: `apiFetch("/x")` targets `/api/x`; root fallback is only GET `/health` (`web/public/api.mjs:11-13`, `web/public/api.mjs:174-185`).
...
- Keep changed-file contract gate clean (`.github/workflows/ci.yml:92-98`, `scripts/check-responses-changed.mjs:275-283`).
