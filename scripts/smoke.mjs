...
function idem() { return `cli-${Date.now()}-${Math.random().toString(16).slice(2)}`; }

(async () => {
  const failures = [];
  async function test(name, fn) {
    try { await fn(); console.log(`✅ ${name}`); }
    catch (e) { console.error(`❌ ${name}:`, e.message); failures.push(name); }
  }
  ...
  await test("POST /generate (txt2img) success shape", async () => {
    ...
    if (!Array.isArray(r.json.data.images) || r.json.data.images.length === 0) throw new Error("images empty");
  });
