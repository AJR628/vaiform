...
async function runAllTests() {
  ...
  console.log(
    `Testing ${testPhrases.length} phrases × ${testFonts.length} fonts × ${testLineSpacings.length} spacings = ${testPhrases.length * testFonts.length * testLineSpacings.length} combinations`
  );
    ...
    for (const phrase of testPhrases) {
      for (const font of testFonts) {
        for (const lineSpacing of testLineSpacings) {
          console.log(
            `Testing: "${phrase}" | ${font.family} ${font.weight} ${font.style} | lineSpacing: ${lineSpacing}`
          );
          ...
          try {
            ...
            console.log(
              `  ${status} | SSIM: ${result.ssim.toFixed(3)} | Diff: ${result.pixelDiff.toFixed(1)}%`
            );
          } catch (error) {
  ...
  if (passed < total) {
    ...
    results
      ...
      .forEach((r) => {
        console.log(
          `  - "${r.phrase}" | ${r.font} | lineSpacing: ${r.lineSpacing} | ${r.error || 'SSIM/Diff failed'}`
        );
      });
  }
