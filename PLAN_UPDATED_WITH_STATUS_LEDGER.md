...
- **C3** CI changed-files contract gate — blocks *new* drift without forcing full-repo cleanup.

### 🟡 In Progress / Partial (implemented but not fully conforming)
- **C4** Story spine contract pass:
  - Most story routes use `ok/fail` and include `requestId`.
  - **Remaining drift:** several safeParse failures pass zod `fieldErrors` arrays into `fields`. Must normalize to `{path: string}`.
- **C5** Assets contract pass:
  - `/api/assets/options` is using canonical envelopes in controller.
  - **Remaining drift:** `/api/assets/ai-images` (disabled) returns 410 **without requestId** and still uses legacy `planGuard('pro')` + `validate(...)` even though it’s disabled.
...
1) **Finish contract conformity for all Active Surfaces** (including disabled endpoints)
2) **SSOT middleware consolidation** (remove “two truths”)
3) **Mount topology cleanup** (single mental model)
...
### C0 — Repo Hygiene (Docs archive + Prettierignore) *(pre-plan but now SSOT)*
**Status:** ✅ DONE  
...
### C1 — Baseline Truth Snapshot (Docs Only)  *(legacy ref: 0.1)*
**Status:** ✅ DONE  
...
### C2 — Cohesion Guardrails Spec (Docs Only)  *(legacy ref: 0.2)*
**Status:** ✅ DONE  
...
### C3 — CI Contract Gate (Changed Files Only)  *(legacy ref: 1.1)*
**Status:** ✅ DONE  
...
### C4 — Story Spine Contract Pass (Routes + Required Middleware)  *(legacy ref: 2.4c)*
**Status:** 🟡 IN PROGRESS (mostly done; fields normalization remains)  
...
5. **Required fix to finish C4:** normalize Zod safeParse errors so `fields` is `{path: string}` (not arrays).
...
### C5 — Assets Contract Pass  *(legacy ref: 2.4d)*
**Status:** 🟡 IN PROGRESS (disabled route drift remains)  
...
### C6 — Post-Spine Truth Refresh (Docs Only)  *(legacy ref: 2.4e)*
**Status:** 🔴 NOT STARTED  
...
### C7 — App Entry Contract Cleanup  *(legacy ref: 1.2)*
**Status:** 🔴 NOT STARTED  
...
### C8 — Active Contract Pass A  *(legacy ref: 1.3)*
**Status:** 🔴 NOT STARTED  
...
### C9 — Active Contract Pass B  *(legacy ref: 1.4)*
**Status:** 🔴 NOT STARTED  
...
### C10 — Auth SSOT Consolidation  *(legacy ref: 2.1)*
**Status:** 🔴 NOT STARTED  
...
### C11 — Plan Guard SSOT Consolidation  *(legacy ref: 2.2)*
**Status:** 🔴 NOT STARTED  
...
### C12 — Validation/Idempotency Dead Code Cleanup  *(legacy ref: 2.3)*
**Status:** 🔴 NOT STARTED  
...
### C13 — Canonical API Router Introduction  *(legacy ref: 3.1)*
**Status:** 🔴 NOT STARTED  
...
### C14 — Remove Duplicate Root API Mounts  *(legacy ref: 3.2)*
**Status:** 🔴 NOT STARTED  
1. Scope: `src/app.js`.
...
### C15 — Credits De-duplication  *(legacy ref: 3.3)*
**Status:** 🔴 NOT STARTED  
...
### C16 — Launch Surface Lock Verification  *(legacy ref: 4.1)*
**Status:** 🔴 NOT STARTED  
...
### C17 — Final Cohesion Publication  *(legacy ref: 4.2)*
**Status:** 🔴 NOT STARTED  
