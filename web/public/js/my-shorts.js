...
async function fetchMine(cursor) {
  const url = cursor
    ? `/api/shorts/mine?limit=24&cursor=${encodeURIComponent(cursor)}`
    : '/api/shorts/mine?limit=24';
...
function cardTemplate(s) {
  ...
  const statusText =
    { ready: 'Ready', processing: 'Processing', failed: 'Failed', error_audio: 'No audio' }[
      status
    ] || 'Unknown';
  return `
...
export async function loadShorts(cursor) {
  ...
  try {
    ...
    items.forEach((s) => {
      if (s.id) myShortsIndex.set(s.id, s);
    });
