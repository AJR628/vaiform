...
function saveCaptionMetaForBeat({ sessionId, beatIndex, captionMeta }) {
  if (!sessionId || typeof beatIndex !== 'number' || beatIndex < 0 || !captionMeta?.lines?.length) {
    console.warn('[caption-meta-handshake] skipped (missing inputs)', {
      sessionId,
      beatIndex,
      hasMeta: !!captionMeta,
    });
...
(function migrateOverlayMetaToV3() {
  ...
  const LEGACY_KEYS = [
    ...
    'overlayMetaSaved',
    'captionMeta',
    'overlayMetaTimestamp',
    '_previewSavedForCurrentText',
  ];
...
export async function generateCaptionPreview(opts) {
  ...
  const ensureFontPx = Number.isFinite(opts?.fontPx)
    ? opts.fontPx
    : typeof window?.getCaptionPx === 'function'
      ? Number(window.getCaptionPx())
      : undefined;
  ...
  const lineSpacingPx =
    fontPx != null
      ? clamp(Math.max(0, Math.round(Math.round(fontPx * lineHeightMul) - fontPx)), 0, MAX_FONT_PX)
      : undefined;
    ...
    overlayV2 && hasRasterFields
      ? (() => {
          const p = {
            ...
            xPx_png: Number.isFinite(opts?.xPx_png)
              ? opts.xPx_png
              : (overlayMeta?.xPx_png ?? xPx_png),
            yPx_png: Number.isFinite(opts?.yPx_png)
              ? opts.yPx_png
              : (overlayMeta?.yPx_png ?? yPx_png),
          ...
          const previewFontStringVal =
            overlayMeta.previewFontString ||
      ...
      : (() => {
          ...
          console.warn(
            '[caption-preview] Building payload without V3 raster fields - overlayV2:',
            overlayV2,
            'hasRasterFields:',
            hasRasterFields
          );
  ...
  if (window.__parityAudit && overlayV2 && hasRasterFields) {
    if (!overlayMeta.yPxFirstLine) {
      console.warn(
        '[__parityAudit] missing yPxFirstLine in overlayMeta; preview request may fail schema'
      );
    }
  ...
  console.log(
    '[caption-overlay] POST /preview/caption with placement:',
    opts.placement,
    'yPct:',
    opts.yPct
  );
  ...
  if (!isOk)
    throw new Error(data?.detail || data?.error || data?.reason || 'Preview generation failed');
  ...
  const imageUrl =
    meta.mode === 'raster' ? meta.rasterUrl || data.data?.imageUrl : data.data?.imageUrl;
  ...
  if (meta.ssotVersion === 3) {
    ...
    if (meta.mode === 'raster') {
      ...
      if (!validateTotalTextHFormula(meta)) {
        console.error(
          '[caption-preview] Server returned invalid totalTextH formula - regenerating preview'
        );
    ...
    normalizedMeta = {
      ...
      yPx_png: Number.isFinite(meta.yPx_png)
        ? meta.yPx_png
        : Number.isFinite(meta.yPx)
          ? meta.yPx
          : undefined,
...
export function getLastCaptionPNG() {
  return lastCaptionPNG;
}
...
function buildBeatPreviewPayload(text, overlayMeta, explicitStyle = {}) {
  const payload = {
    ...
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'fontPx')
      ? { fontPx: overlayMeta.fontPx }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'lineSpacingPx')
      ? { lineSpacingPx: overlayMeta.lineSpacingPx }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'fontFamily')
      ? { fontFamily: overlayMeta.fontFamily }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'weightCss')
      ? { weightCss: overlayMeta.weightCss }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'fontStyle')
      ? { fontStyle: overlayMeta.fontStyle }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'textAlign')
      ? { textAlign: overlayMeta.textAlign }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'letterSpacingPx')
      ? { letterSpacingPx: overlayMeta.letterSpacingPx }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'textTransform')
      ? { textTransform: overlayMeta.textTransform }
      : {}),

    // Color & effects
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'color')
      ? { color: overlayMeta.color }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'opacity')
      ? { opacity: overlayMeta.opacity }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'strokePx')
      ? { strokePx: overlayMeta.strokePx }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'strokeColor')
      ? { strokeColor: overlayMeta.strokeColor }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'shadowColor')
      ? { shadowColor: overlayMeta.shadowColor }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'shadowBlur')
      ? { shadowBlur: overlayMeta.shadowBlur }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'shadowOffsetX')
      ? { shadowOffsetX: overlayMeta.shadowOffsetX }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(explicitStyle, 'shadowOffsetY')
  ...
  if (isPreset && Number.isFinite(overlayMeta?.rasterH) && overlayMeta.rasterH > 0) {
    ...
    const safeTop = Math.round(frameH * 0.1);
    const safeBottom = Math.round(frameH * 0.1);
...
export async function generateBeatCaptionPreview(beatIndex, text, style) {
  ...
  console.log('[beat-preview] Generating preview for beat:', {
    beatId,
...
export function generateBeatCaptionPreviewDebounced(beatIndex, text, style, delay = 300) {
  ...
  if (window.__beatPreviewDebug) {
    console.log('[beat-preview] Debounce triggered:', {
      identifier: id,
      beatIndex: numericBeatIndex,
  ...
  const timer = setTimeout(async () => {
    ...
    if (result && result.rasterUrl) {
      ...
      const beatCardEl =
        document.querySelector(`[data-beat-id="${id}"]`) ||
...
export function applyPreviewResultToBeatCard(beatCardEl, result) {
  ...
  if (window.__beatPreviewDebug) {
    console.log('[beat-preview] Overlay applied:', {
      identifier:
        beatCardEl.getAttribute('data-beat-id') || beatCardEl.getAttribute('data-sentence-index'),
...
export function getSavedOverlayMeta() {
  ...
  try {
    ...
    if (stored) {
      ...
      if (meta.ssotVersion !== 3) {
        console.warn(
          '[caption-preview] Ignoring saved meta with wrong ssotVersion:',
          meta.ssotVersion
        );
...
export function createCaptionOverlay(captionData, container, scaling = {}) {
  ...
  const yPct =
    Number.isFinite(captionData.meta?.yPx_png) && Number.isFinite(captionData.meta?.frameH)
      ? captionData.meta.yPx_png / captionData.meta.frameH
      : Number.isFinite(captionData.meta?.yPct)
        ? captionData.meta.yPct
        : 0.5;
  ...
  let top = Math.max(
    safeTopMargin,
    Math.min(targetTop, finalH - safeBottomMargin - scaledTotalTextH)
  );
  ...
  console.log('[preview-overlay] positioning:', {
    W: finalW,
    H: finalH,
    iw: captionData.meta?.wPx,
    iH: captionData.meta?.hPx,
    rasterW: meta.rasterW,
    rasterH: meta.rasterH,
    hasRaster,
    baseW,
