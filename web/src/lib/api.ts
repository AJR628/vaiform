...
export interface ImageTrack {
  kind: Exclude<BackgroundKind, 'solid' | 'stockVideo'>;
  query?: string | null;
...
export interface StudioState {
  ...
  video?: VideoTrack;
  render: {
    template: 'calm' | 'bold' | 'cosmic' | 'minimal';
    durationSec: number;
    createdAt: string;
    updatedAt: string;
  };
...
export const api = {
  ...
  getUsageLimits: () =>
    req<{ plan: string; isPro: boolean; usage: any; limits: any; resetDate: string }>(
      '/api/limits/usage'
    ),

  generateQuote: (p: { text: string; tone?: string; maxChars?: number }) =>
    req<{
      quote: { id: string; text: string; author?: string; attributed?: boolean; toneTag?: string };
    }>('/api/quotes/generate-quote', { method: 'POST', body: JSON.stringify(p) }),

  remixQuote: (p: {
    originalText: string;
    mode: 'regenerate' | 'rephrase' | 'tone_shift';
    targetTone?: string;
    maxChars?: number;
  }) =>
    req<{
      quote: { id: string; text: string; author?: string; attributed?: boolean; toneTag?: string };
    }>('/api/quotes/remix', { method: 'POST', body: JSON.stringify(p) }),
  ...
  getAssetsOptions: (p: {
    ...
    }>('/api/assets/options', { method: 'POST', body: JSON.stringify(p) }),

  generateAiImages: (p: { prompt: string; style: 'realistic' | 'creative'; count?: number }) =>
    req<{ images: Array<{ id: string; url: string; prompt: string; style: string }> }>(
  ...
  getShort: (jobId: string) => req<any>(`/api/shorts/${encodeURIComponent(jobId)}`),

  diagTtsState: () => req<any>('/diag/tts_state'),
};
