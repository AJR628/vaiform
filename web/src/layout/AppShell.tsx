      ...
      <header className="border-b border-neutral-800">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link
              to="/studio"
              className={`hover:underline ${loc.pathname.startsWith('/studio') ? 'text-white' : 'text-neutral-400'}`}
            >
              Studio
            </Link>
            {String((import.meta as any).env?.VITE_ENABLE_IMAGES_LAB) === 'true' && (
              <Link to="/images" className="text-neutral-400 hover:underline">
                Images
              </Link>
            )}
          </div>
      ...
      {drawer && (
        <div className="fixed right-4 top-14 w-[380px] bg-neutral-900 border border-neutral-800 rounded p-3 space-y-2 shadow-xl">
          ...
          <div className="flex items-center gap-2">
            ...
            {tts && (
              <span
                className={`text-xs px-2 py-0.5 rounded ${tts.configured ? 'bg-green-700' : 'bg-neutral-700'}`}
              >
                {tts.provider}:{tts.configured ? 'on' : 'off'}
              </span>
            )}
          </div>
        </div>
