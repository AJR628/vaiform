...
export function ShortDetailsPage() {
  ...
  useEffect(() => {
    if (!jobId) {
      setState({ loading: false, error: 'Missing jobId' });
      return;
    }
    (async () => {
    ...
    <div className="p-6 space-y-4">
      ...
      {d.videoUrl && (
        <div className="render-stage">
          <video src={d.videoUrl} controls playsInline muted />
        </div>
      )}

      <div className="space-x-3">
        {d.videoUrl && (
          <a className="underline" href={d.videoUrl} target="_blank" rel="noreferrer">
            Open video
          </a>
        )}
        {d.coverImageUrl && (
          <a className="underline" href={d.coverImageUrl} target="_blank" rel="noreferrer">
            Open cover
          </a>
        )}
      </div>
