    ...
    <div className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex gap-2 items-center">
            ...
            <input
              ...
              max={10}
              value={durationSec}
              onChange={(e) => setDurationSec(parseInt(e.target.value) || 8)}
            />
            ...
            <button onClick={start} className="px-3 py-1.5 rounded bg-blue-600 hover:bg-blue-500">
              Start Session
            </button>
          </div>
        </div>
      ...
      {step === 3 && (
        <RenderStep
          ...
          watermark={watermark}
          onDone={() => {
            localStorage.removeItem('vaiform_lastStudio');
          }}
        />
      )}
    </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-2 items-center">
        ...
        />
        <button onClick={gen} className="px-3 py-1.5 rounded bg-neutral-800 hover:bg-neutral-700">
          Generate ×3
        </button>
      </div>
...
function BackgroundStep({
  ...
  const [tab, setTab] = useState<'stockImage' | 'stockVideo' | 'imageUrl' | 'upload' | 'ai'>(
    'stockImage'
  );
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-2">
        <button
          onClick={() => setTab('stockImage')}
          className={`px-2 py-1 rounded ${tab === 'stockImage' ? 'bg-blue-600' : 'bg-neutral-800'}`}
        >
          Stock Image
        </button>
        <button
          onClick={() => setTab('stockVideo')}
          className={`px-2 py-1 rounded ${tab === 'stockVideo' ? 'bg-blue-600' : 'bg-neutral-800'}`}
        >
          Stock Video
        </button>
      </div>
      {tab === 'stockImage' && (
        <div className="space-y-2">
          <div className="flex gap-2 items-center">
            ...
            <select
              value={ken}
              onChange={(e) => setKen(e.target.value as any)}
              className="bg-neutral-900 border border-neutral-800 rounded px-2 py-1"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {cands.map((c) => (
              <div key={c.id} className="border border-neutral-800 rounded overflow-hidden">
                ...
                <button
                  onClick={() => chooseImage(c.id)}
                  className="w-full text-xs px-2 py-1 bg-blue-600 hover:bg-blue-500"
                >
                  Choose
                </button>
              </div>
...
function RenderStep({
  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      try {
        ...
        if (payload.event === 'video_ready' && payload.url) {
        ...
        if (payload.event === 'error') {
          setLoading(false);
          alert(payload.message || 'Render failed');
        }
      } catch {}
    };
    return () => {
  ...
  function pickUrl(obj: any, endsWith: string) {
    try {
      return Object.values(obj).find((u: any) => typeof u === 'string' && u.endsWith(endsWith)) as
        | string
        | undefined;
    } catch {
      return undefined;
  ...
  function chooseMp4(result: any) {
    ...
    const direct =
      typeof result?.url === 'string' && result.url.endsWith('.mp4') ? result.url : undefined;
    ...
    if (nine)
      return {
        src: nine,
        poster: urls[result?.renderId + '_poster_9x16.png'] || pickUrl(urls, '_poster_9x16.png'),
      };
    // fallbacks
    const one =
      pickUrl(urls, '_1x1.mp4') ||
      Object.values(urls).find((u: any) => String(u).endsWith('_1x1.mp4'));
    const wide =
      pickUrl(urls, '_16x9.mp4') ||
      Object.values(urls).find((u: any) => String(u).endsWith('_16x9.mp4'));
  ...
  function handlePreviewPayload(result: any) {
    ...
    if (el) {
      ...
      el.addEventListener(
        ...
        () => {
          try {
            el.currentTime = Math.max(0.001, el.currentTime);
          } catch {}
        },
        { once: true }
      );
  ...
  async function finalize() {
    setLoading(true);
    const r = await api.studioFinalize({
      studioId,
      voiceover,
      wantAttribution,
      captionMode,
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-3">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={wantAttribution}
            onChange={(e) => setWantAttribution(e.target.checked)}
          />
          Attribution
        </label>
        <button onClick={finalize} className="px-3 py-1.5 rounded bg-green-600 hover:bg-green-500">
          Render
        </button>
        {result && (
          <button
      ...
      <div className="border border-neutral-800 rounded p-3 space-y-2">
        <div className="render-stage">
          <video id="preview" controls playsInline preload="metadata" crossOrigin="anonymous" />
        </div>
        {result?.coverImageUrl && (
          <a className="text-blue-400 text-sm" href={result.coverImageUrl} target="_blank">
            Open cover.jpg
          </a>
        )}
      </div>
      {showRemix && <RemixPanel studioId={studioId} onClose={() => setShowRemix(false)} />}
    </div>
  );
}
...
function RemixPanel({ studioId, onClose }: { studioId: string; onClose: () => void }) {
  ...
  async function apply() {
    ...
    if (r.ok)
      setRemixes((prev) => [{ id: (r.data as any).renderId, urls: (r.data as any).urls }, ...prev]);
  }
  return (
      ...
      <div className="ml-auto w-full max-w-md h-full bg-neutral-950 border-l border-neutral-800 p-4 space-y-3 overflow-y-auto">
        ...
        <div className="flex gap-2 text-xs">
          <button
            ...
            className={`px-2 py-1 rounded ${tab === 'background' ? 'bg-neutral-800' : 'bg-neutral-900'}`}
          >
            Background
          </button>
          <button
            onClick={() => setTab('audio')}
            className={`px-2 py-1 rounded ${tab === 'audio' ? 'bg-neutral-800' : 'bg-neutral-900'}`}
          >
            Audio
          </button>
          <button
            onClick={() => setTab('style')}
            className={`px-2 py-1 rounded ${tab === 'style' ? 'bg-neutral-800' : 'bg-neutral-900'}`}
          >
            Style
          </button>
          <button
            onClick={() => setTab('timing')}
            className={`px-2 py-1 rounded ${tab === 'timing' ? 'bg-neutral-800' : 'bg-neutral-900'}`}
          >
            Timing
          </button>
        </div>
        {tab === 'background' && (
          <div className="space-y-2">
            <div className="flex gap-2 items-center">
              <select
                value={bgKind}
                onChange={(e) => setBgKind(e.target.value as any)}
                className="bg-neutral-900 border border-neutral-800 rounded px-2 py-1"
              >
          ...
          <div className="space-x-2">
            {(['Minimal', 'Bold', 'Cinematic'] as const).map((s) => (
              <button
                key={s}
                onClick={() => setStyle(s)}
                className={`px-2 py-1 rounded ${style === s ? 'bg-blue-600' : 'bg-neutral-800'}`}
              >
                {s}
              </button>
            ))}
        ...
        <div className="pt-2 flex justify-between items-center">
          ...
          <button onClick={apply} className="px-3 py-1.5 rounded bg-green-600 hover:bg-green-500">
            Apply
          </button>
        </div>
        {!!remixes.length && (
