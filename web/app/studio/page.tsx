...
export default function StudioPage() {
  ...
  const [quote, setQuote] = useState<{
    candidates: QuoteCand[];
    chosenId?: string;
    iterationsLeft?: number;
  }>({ candidates: [] });
  const [image, setImage] = useState<{
    candidates: ImageCand[];
    chosenId?: string;
    iterationsLeft?: number;
  }>({ candidates: [] });
  ...
  async function resume(id: string) {
    ...
    setQuote({
      candidates: s.quote.candidates || [],
      chosenId: s.quote.chosenId,
      iterationsLeft: s.quote.iterationsLeft,
    });
    setImage({
      candidates: s.image.candidates || [],
      chosenId: s.image.chosenId,
      iterationsLeft: s.image.iterationsLeft,
    });
        ...
        <div className="space-y-3">
          <div className="flex gap-3">
            ...
            <input
              ...
              className="w-24 bg-neutral-900 border border-neutral-800 rounded px-2 py-1"
            />
            <button
              disabled={loading}
              onClick={start}
      ...
      {result && (
        <div className="border border-neutral-800 rounded p-4 space-y-2">
          ...
          {result.coverImageUrl && (
            <img src={result.coverImageUrl} alt="cover" className="w-40 rounded" />
          )}
          <a className="text-blue-400" href={result.videoUrl} target="_blank">
            Open video
          </a>
        </div>
      )}
...
function QuoteStage({
  ...
  studioId: string;
  template: string;
  quote: { candidates: QuoteCand[]; chosenId?: string; iterationsLeft?: number };
  onUpdate: (q: any) => void;
  onNext: () => void;
}) {
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {quote.candidates.map((c) => (
          <div
            key={c.id}
            className={`border rounded p-3 space-y-2 ${quote.chosenId === c.id ? 'border-blue-500' : 'border-neutral-800'}`}
          >
...
function ImageStage({
  ...
  studioId: string;
  image: { candidates: ImageCand[]; chosenId?: string; iterationsLeft?: number };
  onUpdate: (i: any) => void;
  onNext: () => void;
}) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        ...
        <select
          value={kenBurns}
          onChange={(e) => setKenBurns(e.target.value as any)}
          className="bg-neutral-900 border border-neutral-800 rounded px-2 py-1"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {image.candidates.map((c) => (
          <div
            key={c.id}
            className={`border rounded p-2 ${image.chosenId === c.id ? 'border-blue-500' : 'border-neutral-800'}`}
          >
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-3 items-center">
        ...
        <select
          value={captionMode}
          onChange={(e) => setCaptionMode(e.target.value as any)}
          className="bg-neutral-900 border border-neutral-800 rounded px-2 py-1"
        >
