...
export async function nasaSearchVideos({ query, perPage = 12, page = 1 }) {
  ...
  try {
    ...
    console.log(
      `[nasa] raw search: status=${res.status}, total_items=${rawItemsCount}, total_hits=${totalHits}, media_type_filter=video`
    );
    const items = [];
    ...
    for (const item of data?.collection?.items || []) {
      ...
      try {
        ...
        console.log(
          `[nasa] checking asset collection for mp4 (${collection.length} items) for nasa_id=${nasaId}`
        );
        ...
        if (!mp4Url) {
          const sampleHrefs = collection
            .slice(0, 3)
            .map((item) => item?.href?.substring(0, 60) || 'N/A')
            .join(', ');
          console.log(
            `[nasa] drop: no mp4 found in asset list (checked ${collection.length} items, sample hrefs: ${sampleHrefs}...)`
          );
          continue;
        }
    ...
    console.log(
      `[nasa] processed ${processedCount} raw items, returned ${items.length} normalized items (target was ${targetCount})`
    );
