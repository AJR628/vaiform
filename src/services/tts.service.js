...
import {
  ...
  elevenLabsSynthesizeWithTimestamps,
...
export function getLastTtsState() {
  return lastTtsState;
}
...
async function fetchWithRetry(doFetch, key) {
  ...
  while (attempt < 2) {
    ...
    if (status === 429) {
      ...
      const ms = ra
        ? Math.min((parseInt(ra, 10) || 1) * 1000, 2500)
  ...
  return await withAbortTimeout(
    async (signal) => {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          model_id: process.env.ELEVEN_TTS_MODEL || 'eleven_flash_v2_5',
          text,
        }),
        ...(signal ? { signal } : {}),
      });
...
async function synthOpenAI({ text, k }) {
  ...
  const to = setTimeout(
    () => controller.abort(),
    Number(process.env.TTS_FETCH_TIMEOUT_MS || 20_000)
  );
 ...
 */
export async function synthVoiceWithTimestamps({
  text,
  voiceId,
  modelId,
  outputFormat,
  ...
  try {
    ...
    if (!isEleven) {
      console.warn(
        '[tts.timestamps] Only ElevenLabs supports timestamps; falling back to regular synthesis'
      );
    ...
    return await withTtsSlot(async () => {
      try {
        ...
        try {
          ...
          console.log(
            '[tts.timestamps] Synthesis OK:',
            buffer.length,
            'bytes, duration:',
            durationMs ? (durationMs / 1000).toFixed(2) + 's' : 'unknown'
          );
        } catch (err) {
        ...
        if (timestamps && (timestamps.words || timestamps.characters)) {
          ...
          console.log(
            '[tts.timestamps] Text sent to ElevenLabs:',
          ...
          if (words.length > 0) {
            ...
            console.log(
              '[tts.timestamps] First few word timestamps:',
              JSON.stringify(
                firstFew.map((w) => ({
                  word: w.word,
            ...
            console.log(
              '[tts.timestamps] First few character timestamps:',
              JSON.stringify(
                firstFew.map((c) => ({
                  char: c.character,
