...
export async function createShortService({
  ownerUid,
  mode,
  text,
  template,
  ...
  try {
    try {
      console.log('[shorts] render opts:', {
        includeBottomCaption,
        hasCaption: !!caption,
        capPos: caption?.position,
        capAlign: caption?.align,
        capSize: caption?.fontSizePx,
  ...
  try {
    ...
    if (voiceover) {
      try {
        ...
        console.log('[shorts] TTS primary result:', {
          hasAudioPath: !!v?.audioPath,
        ...
        if (!v?.audioPath) {
          ...
          console.log('[shorts] TTS fallback result:', {
            hasAudioPath: !!v?.audioPath,
            audioPath: v?.audioPath,
          });
        }
    ...
    const authorLine =
      wantAttribution && usedQuote.attributed && usedQuote.author ? `— ${usedQuote.author}` : null;
    const watermarkFinal =
    ...
    if (
      captionImage &&
      typeof captionImage === 'string' &&
      captionImage.startsWith('data:image/')
    ) {
      ...
      try {
        ...
        if (karaokeModeEffective === 'ass') {
          ...
          if (overlayCaption?.lines && Array.isArray(overlayCaption.lines)) {
            ...
            console.log(
              `[shorts] Using wrapped text from overlayCaption.lines: ${overlayCaption.lines.length} lines`
            );
            ...
            try {
              ...
              const fontPx =
                overlayCaption?.fontPx || overlayCaption?.sizePx || captionStyle?.fontSizePx || 64;
              ...
              const pad =
                overlayCaption?.internalPaddingPx ??
                overlayCaption?.internalPadding ??
                overlayCaption?.rasterPadding ??
                24;
              ...
              if (letterSpacingPx !== 0) {
                console.warn(
                  `[render-wrap:ssot] letterSpacingPx=${letterSpacingPx} may not match ASS rendering`
                );
              }
    ...
    try {
      if (background?.kind === 'imageUrl' && background?.imageUrl) {
        ...
        if (background.type === 'video' && background.url) {
          try {
            ...
            try {
              ...
              console.error('[background] probe.fail', {
                message: probeErr?.message,
                stderr: String(probeErr?.stderr || '').slice(0, 8000),
              });
              throw probeErr;
            }
            ...
            await renderVideoQuoteOverlay({
              ...
              captionText:
                includeBottomCaption === true ? (caption && caption.text) || usedQuote.text : null,
              captionStyle,
            ...
            console.error('[background] RENDER_FAILED', {
              message: e?.message,
              stack: e?.stack,
          ...
          try {
            const stockUrl =
              background.url || (await resolveStockImage({ query: background.query }));
        ...
        try {
          ...
          if (background.url && background.type === 'image') {
            ...
            console.log(
              `[background] No AI image URL provided, generating new image for prompt: ${background.prompt}`
            );
        ...
        background?.kind === 'stockVideo' &&
        (background?.query || background?.clipUrl || background?.sourceUrl)
      ) {
        try {
          ...
          try {
            await runFFmpeg(['-i', vid.path, '-map', '0:a:0', '-t', '0.1', '-f', 'null', '-']);
            haveBgAudio = true;
          } catch (probeErr) {
            const msg = (probeErr?.stderr || probeErr?.message || '').toString();
            if (
              /Stream specifier matches no streams|Stream map '.+0:a:0'.+`/i.test(msg) ||
          ...
          const keepVideoAudio =
            background.keepVideoAudio !== undefined ? !!background.keepVideoAudio : false;
          const bgAudioVolumeRaw =
            typeof background.bgAudioVolume === 'number'
              ? background.bgAudioVolume
              : haveTTS
                ? 0.25
                : 1.0;
          ...
          const duckDuringTTS =
            keepVideoAudio &&
            (background.duckDuringTTS !== undefined ? !!background.duckDuringTTS : false);
          ...
          console.error('[background] stockVideo render error', {
            message: e?.message,
            stderr: String(e?.stderr || '').slice(0, 8000),
          });
      ...
      try {
        ...
        if (err?.filterComplex)
          errorDetails.filterComplex = String(err.filterComplex).slice(0, 2000);
    ...
    console.log(
      '[shorts] uploading',
      isPng ? 'PNG image' : audioOk ? 'muxed (with audio)' : 'silent',
      isPng ? '' : 'video'
    );
    ...
    try {
      if (imageTmpPath) fs.unlinkSync(imageTmpPath);
    } catch {}
    try {
