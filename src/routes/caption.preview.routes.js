...
const MobileSchema = z
  ...
  .refine((d) => d.placement != null || d.yPct != null, {
    message: 'placement or yPct required',
    path: ['placement'],
  });
    ...
    try {
      ...
      if (!isV3Raster) {
        return fail(
          ...
          400,
          'V3_RASTER_REQUIRED',
          'V3 raster mode is required. Request must include ssotVersion: 3 and mode: "raster"'
        );
      }
      ...
      const useServerMeasure =
        body.measure === 'server' || (body.measure == null && req.get('x-client') === 'mobile');
      ...
      if (!Number.isFinite(rasterW) || rasterW <= 0) {
        ...
        return fail(
          ...
          400,
          'INVALID_RASTER_W',
          `rasterW is required but missing or invalid: ${rasterW}`
        );
      }
      ...
      if (!Number.isFinite(rasterPadding) || rasterPadding < 0) {
        ...
        return fail(
          ...
          res,
          400,
          'INVALID_RASTER_PADDING',
          `rasterPadding is required but missing or invalid: ${rasterPadding}`
        );
      }
      ...
      if (process.env.ALLOW_LEGACY_PREVIEW !== '1') {
        ...
        return fail(
          ...
          'Legacy preview path is disabled. Use V3 raster mode (ssotVersion: 3, mode: "raster")'
        );
      }
      ...
      // Wrap in block scope to isolate variables from V3 path
      {
        ...
        COMPUTED_FIELDS.forEach((k) => {
          if (req.body && k in req.body) {
            console.log(
              `[caption-preview-sanitize] Removing computed field from request: ${k}=${req.body[k]}`
            );
            delete req.body[k];
          }
        });
        ...
        console.log(
          '[caption-preview-wrap] Wrapped into lines:',
          lines.length,
          'from segments:',
          segments.length
        );
        ...
        try {
          ...
          if (Math.abs(totalTextH - expectedTotalTextH) > 0.5) {
            throw new Error(
              `[ssot/v3:INVARIANT] totalTextH=${totalTextH} != expected=${expectedTotalTextH}`
            );
          }
          ...
          if (totalTextH > lines.length * fontPx * 3 || lineSpacingPx > fontPx * 2) {
            ...
            return fail(
              ...
              500,
              'COMPUTATION_ERROR',
              `Metric out of bounds: totalTextH=${totalTextH}, lineSpacingPx=${lineSpacingPx}`
            );
          }
          ...
          console.log('[v3:assert]', {
            ...
            hasAdvancedStyles: Boolean(
              fontStyle !== 'normal' || letterSpacingPx !== 0 || strokePx > 0 || shadowBlur > 0
            ),
          });
      ...
      // Legacy format handling (unreachable if V3 path returns above, but wrapped for scope isolation)
      {
        ...
        console.log(
          `[caption] Derived values: yPctUsed=${yPctUsed}, fontFamilyUsed=${fontFamilyUsed}, weightUsed=${weightUsed}, opacityUsed=${opacityUsed}`
        );
        ...
        console.log(
          `[caption] placement=${placement}, fontPx=${clampedFontPx}, totalTextH=${totalTextH}, yPct=${yPctUsed}, anchorY=${anchorY}, textBlockTop=${textBlockTop}, textStartY=${textStartY}`
        );
        console.log(
          `[caption] effective values: fontFamilyUsed=${fontFamilyUsed}, weightUsed=${weightUsed}, opacityUsed=${opacityUsed}`
        );
...
async function renderCaptionRaster(meta) {
  ...
  const font =
    meta.previewFontString ||
    canvasFontString(meta.weightCss, meta.fontStyle, fontPx, 'DejaVu Sans');
  tempCtx.font = font;
  ...
  for (const line of lines) {
    ...
    if (width > maxLineWidth + 1) {
      // +1px tolerance
      console.warn(
        `[raster] Line exceeds maxLineWidth: "${line}" (${width}px > ${maxLineWidth}px)`
      );
  ...
  if (needsRewrap) {
    ...
    try {
      ...
      serverTotalTextH =
        serverWrappedLines.length * fontPx + (serverWrappedLines.length - 1) * lineSpacingPx;
      ...
      console.warn(
        '[parity:server-rewrap] Using client geometry as fallback due to recomputation error'
      );
    }
  }
  ...
  const previewFontHash = crypto
    .createHash('sha256')
    .update(previewFontString)
    .digest('hex')
    .slice(0, 16);
  ...
  const styleToken =
    normalizeFontStyle(meta.fontStyle || 'normal') === 'italic' ? 'italic' : 'normal';
...
export async function renderPreviewImage(meta) {
  const W = 1080,
    H = 1920; // Standard canvas dimensions
  const canvas = createCanvas(W, H);
  const ctx = canvas.getContext('2d');
  ctx.clearRect(0, 0, W, H);

  // Extract positioning from meta
  const x = Math.round((meta.xPct ?? 0.1) * W);
  const y = Math.round(meta.yPct * H);
  const maxWidth = Math.round((meta.wPct ?? 0.8) * W);
