  ...
  try {
    ...
    if (!f) {
      return res.status(400).json({
        success: false,
        error: 'INVALID_INPUT',
        detail: { fieldErrors: { file: ['File is required'] } },
      });
    }
    ...
    if (!ALLOWED_TYPES.has(type)) {
      return res.status(400).json({
        success: false,
        error: 'INVALID_INPUT',
        detail: { fieldErrors: { file: ['Unsupported MIME type'] } },
      });
    }
    ...
    const payload =
      err?.code === 'LIMIT_FILE_SIZE'
        ? {
            success: false,
            error: 'INVALID_INPUT',
            detail: { fieldErrors: { file: ['File too large (max 8MB)'] } },
          }
...
r.post('/uploads/register', requireAuth, uploadRateLimit, async (req, res) => {
  try {
    ...
    if (!/^https?:\/\//i.test(imageUrl)) {
      return res
        .status(400)
        .json({ success: false, error: 'INVALID_INPUT', message: 'imageUrl required' });
    }
    const jobId = `register-${Date.now()}`;
    const saved = await saveImageFromUrl(ownerUid, jobId, imageUrl, {
      index: 0,
      recompress: false,
    });
    ...
    return res
      .status(500)
      .json({ success: false, error: 'REGISTER_FAILED', message: e?.message || 'failed' });
  }
});
