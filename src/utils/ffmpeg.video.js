...
export async function renderVideoQuoteOverlay({
  ...
  if (caption && String(caption.text || '').trim()) {
    ...
    // per-line drawtext (centered x) — escape commas so ffmpeg never splits filters
    ...
    for (let i = 0; i < n; i++) {
      ...
      capDraws.push(
        `drawtext=text='${escLine}':fontfile='${fontFile}':x=${xFinal}:y=${yExpr}:fontsize=${fontPx}:fontcolor=white@${op.toFixed(2)}:line_spacing=0:fix_bounds=1:borderw=2:bordercolor=black@0.85:shadowcolor=black:shadowx=2:shadowy=2:box=${wantBox?1:0}:boxcolor=black@${boxAlpha.toFixed(2)}:boxborderw=0`
      );
    }
