...
async function getImageDimensions(imagePath) {
  try {
    ...
    return new Promise((resolve, reject) => {
      const proc = spawn(
        ffprobePath,
        ['-v', 'error', '-show_entries', 'stream=width,height', '-of', 'csv=p=0:s=x', imagePath],
 ...
 */
export async function renderSolidQuoteVideo({
  outPath,
  text,
  durationSec = 8,
  template = 'minimal',
  ...
  const args = [
    ...
    'lavfi',
    '-i',
    `color=c=${bg}:s=${width}x${height}:d=${durationSec}`,
    '-vf',
    filter,
    '-r',
    String(fps),
...
export async function renderImageQuoteVideo({
  ...
  if (overlayCaption && (overlayCaption.ssotVersion === 2 || overlayCaption.ssotVersion === 3)) {
    console.log(
      '[renderImageQuoteVideo] Using SSOT v' + overlayCaption.ssotVersion + ' overlayCaption'
    );
  ...
  if (overlayCaption?.mode === 'raster') {
    ...
    try {
      ...
      console.error(
        '[renderImageQuoteVideo:raster] Failed to materialize PNG overlay:',
        error.message
      );
    ...
    try {
      ...
      if (fs.existsSync(pngPath) && fs.statSync(pngPath).size > 0) {
        ...
        console.log(
          '[renderImageQuoteVideo] USING OVERLAY - skipping drawtext. Caption PNG:',
          pngPath
        );
      } else {
      ...
      console.warn(
        '[renderImageQuoteVideo] failed to save or verify caption PNG, falling back to drawtext:',
        error.message
      );
  ...
  if (usingCaptionPng && (!captionPngPath || !fs.existsSync(captionPngPath))) {
    console.warn(
      '[renderImageQuoteVideo] Caption PNG path invalid or missing, falling back to drawtext'
    );
    console.warn('[renderImageQuoteVideo] PNG path was:', captionPngPath);
    console.warn(
      '[renderImageQuoteVideo] File exists:',
      captionPngPath ? fs.existsSync(captionPngPath) : false
    );
    usingCaptionPng = false;
    captionPngPath = null;
  }
  ...
  if (usingCaptionPng && captionPngPath && fs.existsSync(captionPngPath)) {
    ...
    if (stats.size === 0) {
      ...
      console.log(
        `[renderImageQuoteVideo] Caption PNG verified: ${captionPngPath} (${stats.size} bytes)`
      );
    }
  }
  ...
  if (assPath) {
    ...
    if (usingSSOT && overlayCaption) {
      if (Number.isFinite(overlayCaption.yPxFirstLine)) {
        ...
        Number.isFinite(overlayCaption.yPct) &&
        Number.isFinite(overlayCaption.totalTextH)
      ) {
        ...
        console.log('[renderImageQuoteVideo] Calculated from SSOT yPct:', {
          yPct,
          totalTextH,
          targetTop,
          yPosition,
  ...
  if (usingCaptionPng && captionPngPath && fs.existsSync(captionPngPath)) {
    ...
    if (overlayCaption?.mode === 'raster') {
      ...
      if (Number.isFinite(overlayCaption.xPx_png)) {
        ...
        console.log('[renderImageQuoteVideo:raster:x-absolute]', {
          xPx_png: overlayCaption.xPx_png,
          overlayX,
        });
      } else {
        overlayX = (overlayCaption.xExpr_png || overlayCaption.xExpr || '(W-overlay_w)/2').replace(
          /\s+/g,
          ''
        );
      ...
      if (Number.isFinite(overlayCaption.yPxFirstLine)) {
        ...
        overlayY = Math.max(
          safeTopMargin,
          Math.min(targetTop, H - safeBottomMargin - totalTextH)
        ).toString();
        console.log(
          `[renderImageQuoteVideo] SSOT v2 yPct calculation: yPct=${yPct}, totalTextH=${totalTextH}, overlayY=${overlayY}`
        );
      } else {
        overlayY = '0';
      }
      ...
      console.log(
        `[renderImageQuoteVideo] Using legacy SSOT coordinates: xPx=${overlayX}, yPx=${overlayY}`
      );
    } else {
      // Default fallback
      ...
      overlayY = '0';
      console.log(
        `[renderImageQuoteVideo] Using default positioning: x=${overlayX}, y=${overlayY}`
      );
    }
  ...
  const isStillImage =
    !hasTTS && usingCaptionPng && captionPngPath && fs.existsSync(captionPngPath);
  ...
  if (isStillImage && outExt === '.mp4') {
    ...
    console.log(
      `[renderImageQuoteVideo] Changing output from .mp4 to .png for still image: ${finalOutPath}`
    );
  }
  const isImageFormat =
    isStillImage &&
    (outExt === '.png' || outExt === '.jpg' || outExt === '.jpeg' || outExt === '.mp4');
  ...
  if (isImageFormat) {
    ...
    args = [
      ...
      fs.statSync(captionPngPath).size > 0
        ? ['-i', captionPngPath]
        : []),
      ...(usingCaptionPng && captionPngPath && fs.existsSync(captionPngPath)
    ...
    console.log(
      `[render.image] Still image mode: inputs img=${imagePath} cap=${usingCaptionPng ? captionPngPath : 'none'}`
    );
  } else {
    // Render as MP4 video
    ...
    if (hasTTS) {
      console.log(
        `[audio.map] TTS input index: ${ttsInputIndex}, map="[vout]" + "${ttsInputIndex}:a" aac 192k`
      );
    } else {
  ...
  console.log(
    `[render.image] inputs: img=${imagePath} cap=${usingCaptionPng ? captionPngPath : 'none'} tts=${hasTTS ? ttsPath : 'none'}`
  );
