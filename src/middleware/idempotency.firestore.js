    ...
    try {
      ...
      if (prepared.kind === 'enqueued') {
        ...
        emitFinalizeEvent('info', FINALIZE_EVENTS.JOB_CREATED, {
          ...
          sessionId,
          attemptId: key,
          finalizeJobId: prepared.attempt?.jobId ?? key,
          executionAttemptId: prepared.attempt?.executionAttemptId ?? null,
          stage: FINALIZE_STAGES.QUEUE_ENQUEUE,
          jobState: prepared.attempt?.jobState ?? prepared.attempt?.state ?? 'queued',
