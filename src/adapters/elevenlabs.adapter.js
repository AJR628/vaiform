 ...
 */
export async function elevenLabsSynthesizeWithTimestamps({
  text,
  voiceId,
  modelId,
  outputFormat,
  ...
  return await withAbortTimeout(
    async (signal) => {
      ...
      console.log(
        '[elevenlabs.timestamps] Has alignment:',
        !!data.alignment,
        'type:',
        typeof data.alignment
      );
      console.log(
        '[elevenlabs.timestamps] Has normalized_alignment:',
        !!data.normalized_alignment,
        'type:',
        typeof data.normalized_alignment
      );
      ...
      if (data.alignment) {
        if (Array.isArray(data.alignment)) {
          ...
          if (data.alignment.length > 0) {
            console.log(
              '[elevenlabs.timestamps] Sample alignment entry:',
              JSON.stringify(data.alignment[0])
            );
          }
        } else if (typeof data.alignment === 'object') {
          console.log('[elevenlabs.timestamps] Alignment keys:', Object.keys(data.alignment));
          console.log(
            '[elevenlabs.timestamps] Alignment preview:',
            JSON.stringify(data.alignment).substring(0, 500)
          );
        } else {
          console.log(
            '[elevenlabs.timestamps] Alignment value:',
            String(data.alignment).substring(0, 200)
          );
        }
      }
      ...
      if (data.alignment) {
        if (Array.isArray(data.alignment)) {
          ...
          for (const entry of data.alignment) {
            if (
              entry.character &&
              (entry.start_time_ms !== undefined || entry.start !== undefined)
            ) {
          ...
          if (
            ...
            console.log(
              '[elevenlabs.timestamps] Parsed character timestamps from separate arrays:',
              characters.length
            );
      ...
      if (characters.length === 0 && words.length === 0 && data.normalized_alignment) {
        if (Array.isArray(data.normalized_alignment)) {
          ...
          if (
            Array.isArray(data.normalized_alignment.characters) &&
            ...
            console.log(
              '[elevenlabs.timestamps] Parsed character timestamps from normalized_alignment arrays:',
              characters.length
            );
      ...
      if (characters.length > 0 || words.length > 0) {
        console.log(
          '[elevenlabs.timestamps] Extracted timestamps - characters:',
          characters.length,
          'words:',
          words.length
        );
        ...
        console.warn(
          '[elevenlabs.timestamps] WARNING: No timestamps found in response. Check logs above for actual structure.'
        );
      }
