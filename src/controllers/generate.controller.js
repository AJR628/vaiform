...
export async function generate(req, res) {
  try {
    ...
    if (!adapter || typeof adapter.invoke !== 'function') {
      ...
      return res
        .status(500)
        .json({ success: false, error: `No adapter found for style "${style}".` });
    }
    ...
    for (let i = 0; i < n; i++) {
      ...
      try {
        ...
        dlog('adapter.invoke ok', {
          i,
          hasPredictionUrl: !!predictionUrl,
          hasDirect: !!directOutput,
        });
      } catch (e) {
    ...
    await db.collection('users').doc(email).collection('generations').doc().set({
      prompt,
      urls: outputUrls,
      style,
      type: 'text-to-image',
